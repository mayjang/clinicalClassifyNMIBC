...
safe_best_thr_youden <- function(y_fac, p) {
  ok <- is.finite(p) & !is.na(y_fac)
  y2 <- y_fac[ok]; p2 <- p[ok]
  if (length(unique(y2)) < 2) return(NA_real_)
  r <- pROC::roc(ifelse(y2 == "Rec", 1, 0), p2, quiet = TRUE)
  as.numeric(pROC::coords(r, x = "best", best.method = "youden", transpose = FALSE)["threshold"])
}


if (has_kn) {
  thr_test_like_first <- safe_best_thr_youden(y_test, probs_all[[main_model]]$test)
  
  thr_nm_test_like_first <- safe_best_thr_youden(y_test, u_nm_score)

  p_my_kn <- probs_all[[main_model]]$knowles
  mets_my_kn <- core_metrics(k_y_fac, p_my_kn, thr_test_like_first)

  p_nm_kn <- k_nm_score
  mets_nm_kn <- core_metrics(k_y_fac, p_nm_kn, thr_nm_test_like_first)
...
saveRDS(list(
  ...
  # Feature matrices (clinical + expression)
  X_train_clin_base = X_train_clin_base,
  X_test_clin_base  = X_test_clin_base,
  X_train_de_cand   = X_train_de_cand,
  X_test_de_cand    = X_test_de_cand,
  X_train_combo_all = X_train_combo_all,
  X_test_combo_all  = X_test_combo_all,
  X_train_panel     = X_train_panel,
  X_test_panel      = X_test_panel,
  X_kn_clin_base    = X_kn_clin_base,
  X_kn_de_cand      = X_kn_de_cand,
  X_kn_combo_all    = X_kn_combo_all,
  X_kn_panel        = X_kn_panel,

  # Models
  m_clin_base = m_clin_base,
  m_de_cand   = m_de_cand,
  m_combo_en  = m_combo_en,
  m_combo_lasso_panel = m_combo_lasso_panel,
  ...
  # NMIBC baseline outputs
  u_nm_score = u_nm_score,
  k_nm_score = k_nm_score,
  u_nm_score_pred = u_nm_score_pred,
  k_nm_score_pred = k_nm_score_pred,

  # Raw NMIBC classify outputs 
  nm_u_test = nm_u_test,
  nm_k      = nm_k,
  col_class = col_class
), file = "classifier_objects.rds")
